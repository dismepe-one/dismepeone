(function(){
  ...
  const isIndustryOnly = user => ['INDUSTRIA','COMPRADOR'].includes(String(user?.tipo||'').trim().toUpperCase());
  const go = () => {
    ...
    try{
      ...
      if(response.ok && /\/auth\/login(?:\?|$)/.test(url)){
        response.clone().json().then(data=>{
          if(isIndustryOnly(data?.usuario)) go();
        }).catch(()=>{});
      }
    }catch(e){}
    return response;
  ...
  originalFetch('/auth/me?industries_router='+Date.now(), {credentials:'include', cache:'no-store'})
    ...
    .then(data=>{ if(isIndustryOnly(data?.usuario)) go(); })
    .catch(()=>{});
})();
