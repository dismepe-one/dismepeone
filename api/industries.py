...
async def industries_page(
    ...
    html = html.replace(
        ...
        "<script id=\"industry-sales-projection-v1\">\n(function(){\n  'use strict';\n  if (typeof renderKpis !== 'function') return;\n  const originalRenderKpis = renderKpis;\n  const currency = v => Number(v).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});\n  const number = v => Number(v).toLocaleString('pt-BR',{maximumFractionDigits:1});\n  function recifeDate(value){\n    // Venda Geral entrega \"dd/mm/aaaa hh:mm:ss\" em horário local.\n    const brazil=/^(\\d{2})\\/(\\d{2})\\/(\\d{4})(?:\\s+\\d{2}:\\d{2}(?::\\d{2})?)?$/.exec(String(value||'').trim());\n    if(brazil){\n      const day=Number(brazil[1]), month=Number(brazil[2]), year=Number(brazil[3]);\n      const check=new Date(Date.UTC(year,month-1,day));\n      return check.getUTCFullYear()===year&&check.getUTCMonth()===month-1&&check.getUTCDate()===day\n        ? {year,month,day}:null;\n    }\n    const date = new Date(value);\n    if(!Number.isFinite(date.getTime()))return null;\n    const parts = new Intl.DateTimeFormat('en-US',{\n      timeZone:'America/Recife',year:'numeric',month:'2-digit',day:'2-digit'\n    }).formatToParts(date);\n    const read = type => Number(parts.find(p=>p.type===type)?.value);\n    const year=read('year'), month=read('month'), day=read('day');\n    return year&&month&&day ? {year,month,day} : null;\n  }\n  function businessDays(year,month,throughDay){\n    let count=0;\n    for(let day=1;day<=throughDay;day++){\n      const weekday=new Date(Date.UTC(year,month-1,day)).getUTCDay();\n      if(weekday!==0&&weekday!==6)count++;\n    }\n    return count;\n  }\n  function addProjection(view){\n    const host=document.getElementById('homePerformance');\n    if(!host)return;\n    const active=view||document.querySelector('.tab.active')?.dataset.view||'inicio';\n    if(active!=='inicio')return;\n    const data=typeof salesData!=='undefined'?salesData:null;\n    if(!data||data.todosLaboratorios===true)return;\n    const comp=/^(0[1-9]|1[0-2])\\/(20\\d{2})$/.exec(String(data.competencia||''));\n    const summary=data.resumo||{};\n    const sale=summary.vendaTotal, goal=summary.objetivoTotal;\n    const source=recifeDate(summary.vendaTotalAtualizadoEm);\n    const now=recifeDate(new Date().toISOString());\n    const card=document.createElement('section');\n    card.id='industrySalesProjection';\n    card.setAttribute('aria-label','Projeção de vendas do laboratório');\n    card.style.cssText='margin-top:12px;padding:14px;border:1px solid #c8e5d6;border-radius:13px;background:linear-gradient(135deg,#f0fbf5,#ffffff);color:#173b2d;';\n    const heading=document.createElement('div');\n    heading.style.cssText='font-size:12px;font-weight:900;margin-bottom:7px;';\n    heading.textContent='Projeção de vendas até o fechamento do mês';\n    card.appendChild(heading);\n    const main=document.createElement('div');\n    main.style.cssText='font-size:23px;font-weight:900;line-height:1.3;';\n    card.appendChild(main);\n    const detail=document.createElement('div');\n    detail.style.cssText='font-size:11px;line-height:1.6;margin-top:6px;color:#526b5d;';\n    card.appendChild(detail);\n    const note=document.createElement('div');\n    note.style.cssText='font-size:10px;line-height:1.5;margin-top:9px;color:#657b70;';\n    card.appendChild(note);\n    host.appendChild(card);\n    const valid=comp&&sale!==null&&sale!==undefined&&Number.isFinite(Number(sale))&&Number(sale)>=0\n      &&source&&now&&source.year===Number(comp[2])&&source.month===Number(comp[1])\n      &&now.year===source.year&&now.month===source.month\n      &&(source.day<now.day||(source.day===now.day));\n    if(!valid){\n      main.textContent='—';\n      detail.textContent='Projeção indisponível para a competência ou fonte selecionada.';\n      note.textContent='Exige Venda Geral e data de atualização válidas na competência atual.';\n      return;\n    }\n    const lastDay=new Date(Date.UTC(source.year,source.month,0)).getUTCDate();\n    const elapsed=businessDays(source.year,source.month,source.day);\n    const total=businessDays(source.year,source.month,lastDay);\n    if(!elapsed||!total){\n      main.textContent='—';\n      detail.textContent='Ainda não há dias úteis suficientes para calcular a projeção.';\n      return;\n    }\n    const projection=Number(sale)/elapsed*total;\n    if(!Number.isFinite(projection)){\n      main.textContent='—';\n      detail.textContent='Não foi possível calcular a projeção com a fonte disponível.';\n      return;\n    }\n    main.textContent=currency(projection);\n    const hasGoal=goal!==null&&goal!==undefined&&Number.isFinite(Number(goal))&&Number(goal)>0;\n    detail.textContent='Venda Geral realizada: '+currency(sale)\n      +(hasGoal?' · Meta: '+currency(goal)+' · Projeção da meta: '+number(projection/Number(goal)*100)+'%':'')\n      +' · Dias úteis considerados: '+elapsed+' de '+total+'.';\n    note.textContent='Estimativa linear sobre a Venda Geral registrada até '+String(source.day).padStart(2,'0')+'/'+String(source.month).padStart(2,'0')+'/'+source.year\n      +'. Considera segunda a sexta, sem feriados ou sazonalidade; não representa venda realizada nem garantia de resultado.';\n  }\n  renderKpis=function(view){\n    const result=originalRenderKpis.apply(this,arguments);\n    try{addProjection(view);}catch(e){console.warn('[Industria projeção]',e);}\n    return result;\n  };\n})();\n</script>\n"
        '</body>',
        1,
    )
    return HTMLResponse(
